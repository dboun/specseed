...
class TrackingRemoteGitHub(TrackingBase):
    ...
    def delete_entry(self, entry_id: int | str) -> TrackingResult:
        # The REST API cannot delete an issue; the GraphQL deleteIssue mutation
        # can, but only with sufficient permissions (typically maintain/admin).
        # On a permission failure this returns ok=False; callers that just want
        # the issue gone from the active set should set_entry_closed instead.
        try:
            issue = self._request("GET", f"/repos/{self.repo}/issues/{entry_id}")
            self._graphql(
    ...
    def edit_entry(
        ...
        try:
            ...
            if body is not None:
                payload["body"] = body
            issue = self._request("PATCH", f"/repos/{self.repo}/issues/{entry_id}", body=payload)
            return TrackingResult(ok=True, data=TrackingEntryId(id=issue["number"]))
        except Exception as exc:
            return self._error(exc)
    ...
    def remove_entry_label(self, entry_id: int | str, label: str) -> TrackingResult:
        try:
            ...
            try:
                labels = self._request(
                    "DELETE", f"/repos/{self.repo}/issues/{entry_id}/labels/{quoted}"
                )
            except GitHubRemoteError as exc:
                # 404 means the label was not on the issue: idempotent success.
                if exc.status != 404:
                    raise
                issue = self._request("GET", f"/repos/{self.repo}/issues/{entry_id}")
            ...
            return TrackingResult(
                ...
                data=TrackingLabelSet(
                    entry_id=entry_id,
                    labels=[self._label_from_github(item) for item in labels or []],
                ),
            )
        except Exception as exc:
            return self._error(exc)
    ...
    def list_pull_requests(
        ...
        try:
            state = "all" if is_open is None else ("open" if is_open else "closed")
            pulls = self._paginate(f"/repos/{self.repo}/pulls", {"state": state})
            summaries = [self._summary_from_pull_request(pr) for pr in pulls]
            if labels:
                wanted = set(labels)
                summaries = [pr for pr in summaries if wanted.issubset({label.name for label in pr.labels})]
            if assignee is not None:
                summaries = [pr for pr in summaries if assignee in pr.assignees]
            if updated_since is not None:
                summaries = [pr for pr in summaries if pr.updated_at is not None and pr.updated_at >= updated_since]
            return TrackingResult(ok=True, data=summaries)
        except Exception as exc:
            return self._error(exc)

    def get_pull_request(self, pull_request_id: int | str) -> TrackingResult:
        try:
            pr = self._request("GET", f"/repos/{self.repo}/pulls/{pull_request_id}")
            summary = self._summary_from_pull_request(pr)
            comments = [
                self._comment_from_issue_comment(comment)
                for comment in self._paginate(
                    f"/repos/{self.repo}/issues/{pull_request_id}/comments"
                )
            ]
            return TrackingResult(
                ok=True,
    ...
    def add_pull_request(
        ...
        try:
            ...
            if body is not None:
                payload["body"] = body
            pr = self._request("POST", f"/repos/{self.repo}/pulls", body=payload)
            number = pr["number"]
            if labels:
                self._request(
                    "POST",
            ...
            return TrackingResult(ok=True, data=TrackingPullRequestId(id=number))
        except Exception as exc:
            return self._error(exc)
    ...
    def set_pull_request_closed(self, pull_request_id: int | str) -> TrackingResult:
        try:
            pr = self._request(
                "PATCH",
                f"/repos/{self.repo}/pulls/{pull_request_id}",
                body={"state": "closed"},
            )
            return TrackingResult(
                ok=True,
    ...
    def _summary_from_pull_request(self, pr: dict[str, object]) -> TrackingPullRequestSummary:
        return TrackingPullRequestSummary(
            ...
            labels=[self._label_from_github(label) for label in pr.get("labels", [])],
            is_open=pr.get("state") == "open",
            is_merged=bool(pr.get("merged")),
            url=pr.get("html_url"),
            author=self._login(pr.get("user")),
            assignees=[self._login(user) for user in pr.get("assignees", []) if self._login(user)],

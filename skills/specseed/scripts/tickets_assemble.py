...
- status: auto-advances to `done` when a ticket has issues and ALL are done
  (unless deprecated/forced). Live status is otherwise preserved from an
  existing tickets.json when the folder is still at seed `todo` (see
  issues_assemble.py for the same source-of-truth split). --no-preserve opts out.
...
def main():
    ...
    # Pre-aggregate per-ticket issue effort + completion from issues.json
    effort_by_ticket, total_by_ticket, done_by_ticket = {}, {}, {}
    if issues:
        for iid, ie in issues.items():
            ...
            if isinstance(eh, bool) or not isinstance(eh, (int, float)):
                eh = 0
            effort_by_ticket[parent] = effort_by_ticket.get(parent, 0) + eh
            total_by_ticket[parent] = total_by_ticket.get(parent, 0) + 1
            if ie.get("status") in DONE_STATES:
                done_by_ticket[parent] = done_by_ticket.get(parent, 0) + 1
    ...
    for folder in sorted(tickets_dir.iterdir()):
        ...
        # Derived fields
        meta["effort_hours"] = effort_by_ticket.get(tid, 0)
        total = total_by_ticket.get(tid, 0)
        done = done_by_ticket.get(tid, 0)
        meta["issues_total"] = total
        meta["issues_done"] = done
        # Auto-advance to done when all issues are done (unless deprecated/forced)

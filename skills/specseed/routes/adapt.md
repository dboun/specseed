...
- Add new tickets/issues if scope expanded (use `references/work-breakdown.md`) — new folders + back-links (ticket `issues:` ⟷ issue `ticket:`); add titles to `ROADMAP.md`. **Run the risk-detection & gating pass** (`work-breakdown.md`) over any newly-formed issues — flag gated actions, get explicit coverage approval, set per-issue `approval_required`, propose isolate-gated-execution splits

...
**Copied-config / partial-mirror case.** `config.json` is PORTABLE; `remote.json` is NOT. So a user who DROPPED IN a `config.json` (with `backend.enabled:true`) into a fresh repo has the "how-you-work" half but no per-repo mirror state. Detect this: `config.json` says a mirror is on but `remote.json` is missing or has no `repo` → don't try to scaffold/mirror yet; route into **configure mode** to fill the per-repo half (point them at `configure.py`, which collects the repo URL + verifies the PAT, writes `remote.json`). Then proceed as the configure preamble does (scaffold if opted, continue into bootstrap/adopt).

**Scaffolded empty remote.** A mirror that was SCAFFOLDED at configure time (its `remote.json` has `scaffolded:true`, `initialized:false`, empty dashboards) is still **config/state, not a spec** — it does NOT change routing. No `.specseed/spec/` content → still **bootstrap** (case 6). The first spec can be driven locally OR async from a `bootstrap`-labeled remote issue (the runner relays it as a `kind:bootstrap` request — see `references/remote.md`).
...
| **tweak** | Tiny single-doc edit ("add this one req to SRS", "change priority of REQ-X", "remember this repo instruction") | `routes/tweak.md` (may auto-escalate to adapt) |
...
- `routes/migrate.md` — the `migrate` route: bring an older-version `.specseed/` tree up to the running skill's format. Reads the skill's `version.txt` + the tree's `.specseed/version.txt`, applies the in-range files from `migrations/`. See `migrations/README.md` for the x.y.z model + file format. Auto-offered by the reconnaissance version check
...
- `repo/index.md` — **repo-specific standing instructions**, read on every specseed session and by implementation agents. Holds only durable rules for this repository: custom validation plans, project-specific agent policy, doc maintenance obligations, local process constraints. It may link sibling markdown files for scenario-specific detail. Read those linked files only when relevant.

**Repo memory structure:**
- `.specseed/memory/repo/index.md` is the always-read entry point. Keep it short enough to load every time.
- Sibling files next to it (`verification.md`, `release.md`, `data-policy.md`, etc.) hold conditional detail. Link them from `index.md` with when-to-read notes.
- Do NOT put session scratch, sprint preferences, config, credentials, mirror state, or one-off decisions here.
- When the user gives arbitrary custom instructions for a repo, split them: always-needed rules → `repo/index.md`; conditional detail → sibling files linked from index.
...
│   ├── repo/                   #   repo-specific standing instructions (durable, not portable)
│   │   ├── index.md            #     ALWAYS read: custom repo rules + links to scenario docs
│   │   └── *.md                #     OPTIONAL scenario docs, read only when index says relevant

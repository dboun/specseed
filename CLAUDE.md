...
- **Memory** lives under `.specseed/memory/` (dir): `session_state.md` (session scratch) + `sprint_planning.md` (durable sprint prefs). Co-located CONFIG (not memory): `config.json` — the PORTABLE "how-you-work" file (HITL gates + git workflow + `backend{enabled,provider}` + `runner{}` incl. the `agents{}` matrix [FUNCTION implement/review/qa → DIFFICULTY easy/hard → ordered fallback chain of {provider claude|codex, config_dir, model, effort} specs] + `review{}` code-review gate + `qa{}` policy); copyable between repos — and `remote.json` — per-repo mirror STATE (repo/allowlist/issue-map/cursors), NOT portable, mirror only. Split rule: anything project-specific → remote.json; the rest → config.json. `save_state` persists only the state keys so transient runtime fields never leak.

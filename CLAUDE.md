...
agent-output) `spec/` (live spec) `spec-change/<id>/` (staged spec + plan.json)
...
applies remote post mutations from JSON. The worker never enqueues, never runs generated code, never touches git/code.
...
- **executing/dispatch.py** - switches on `task["action"]`: `apply_spec_change_plan` (runtime JSON
  executor), vs work handlers (read entity,
...
- **plan-first approval / settle-on-approval** (`executing/dispatch.propose_spec_change` +
  ...
  `plan.json` + STAGED spec, then STOPS - it never enqueues and never picks whether the run
  ...
  creates the epics/tickets/issues (issues born `todo`). The RUNTIME closes the request in code after apply succeeds
